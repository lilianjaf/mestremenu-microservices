...
class ApenasDonoPodeDeletarTipoUsuarioRuleTest {

    private final ApenasDonoPodeDeletarTipoUsuarioRule rule = new ApenasDonoPodeDeletarTipoUsuarioRule();

    private ExclusaoTipoUsuarioContext contexto(UsuarioBase logado) {
        TipoUsuario tipo = new TipoUsuario("GERENTE", TipoNativo.CLIENTE);
        return new ExclusaoTipoUsuarioContext(Optional.of(tipo), () -> false, logado);
    }
    ...
    void deveLancarExcecaoQuandoNaoEhDono() {
        ...
        assertThrows(AcessoNegadoExclusaoTipoUsuarioException.class, () -> rule.validar(contexto(naoEhDono)));
    }
}
